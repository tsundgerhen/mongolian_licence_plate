...
# ========== Setup ==========
pytesseract.pytesseract.tesseract_cmd = r"/opt/homebrew/bin/tesseract"  # Modify this path if needed
...
def fix_plate_format(plate_text):
    plate_text = plate_text.replace('О', 'O').replace('З', '3')
...
def get_most_common_plate(track_id, history):
    fixed_texts = [fix_plate_format(t) for t in history[track_id] if len(fix_plate_format(t)) == 7]
...
while cap.isOpened():
    ...
    for plate, track_id in plate_to_track_id.items():
        ...
        digits_text = pytesseract.image_to_string(thresh, config='--psm 7 -c tessedit_char_whitelist=0123456789').strip()
        ...
        for r in char_results:
            for box in r.boxes:
                cls = int(box.cls[0])
                if cls >= 10:  # Non-digit characters only
                    cx = int((box.xyxy[0][0] + box.xyxy[0][2]) / 2)
                    char_list.append((cx, char_map.get(cls, '?')))

        char_list.sort(key=lambda x: x[0])
        char_text = ''.join(c[1] for c in char_list)

        plate_text = digits_text + char_text
        if plate_text:
